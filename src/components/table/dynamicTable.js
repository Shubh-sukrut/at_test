...
export default function DynamicTable({ columns, rows, status, page, setPage, totalPage, rowsPerPage, setRowsPerPage }) {
    ...
    useEffect(() => {
        const list = stableSort(rows, getComparator(order, orderBy))
        setVisibleRows(list)
                ...
                rows?.length ? <>
                    <Container>
                        <Paper sx={{ width: '100%', overflow: 'hidden' }}>
                                                    ...
                                                    <TableRow hover key={index} sx={{ cursor: 'pointer' }} >
                                                        {
                                                            columns?.map((column) => {
                                                                ...
                                                                const textTransform = column?.textTransform ? column.textTransform : 'capitalize';

                                                                if (column.format && typeof value === 'number') {
                                                                    return (
                                                                        <TableCell key={column.id} align={column.align} style={{ textTransform: textTransform }}>
                                                                            {column.format(value)}
                                                                        </TableCell>
                                                                    );
                                                                } else {
                            ...
                            <TablePagination
                                page={page}
                                count={totalPage ? totalPage : rows.length}
                                component="div"
